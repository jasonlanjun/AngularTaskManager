...
export class ProjectEffects {
  ...
  loadProjects$: Observable<Action> = this.actions$
    .pipe(
      ...
      switchMap(([_, auth]) => this.service.get(<string>auth.user.id)
        .pipe(
  ...
  updateLists$: Observable<Action> = this.actions$
    .pipe(
      ...
      map(action => action.payload),
      switchMap(project => this.service.updateTaskLists(project)
        .pipe(
  ...
  addTaskFilterId$: Observable<Action> = this.actions$
    .pipe(
      ...
      map(action => action.payload),
      switchMap(project => this.service.insertTaskFilter(project)
        .pipe(
          map(p => new actions.InsertFilterSuccessAction(p)),
          catchError(err => of(new actions.InsertFilterFailAction(JSON.stringify(err))))
        )
      )
    );
  ...
  addUserPrjRef$: Observable<Action> = this.actions$
    .pipe(
      ...
      withLatestFrom(this.store$.pipe(
        ...
        map(auth => auth.user)), (projectId: string, user: User) => {
